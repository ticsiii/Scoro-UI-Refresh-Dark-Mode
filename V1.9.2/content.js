 ...
 *                             (tomorrow onward; the current day is never warned —
 *                             local date-string compare to avoid UTC offset issues).
...
function initTimesheetFutureWarning() {
  ...
  const checkEntry = (input) => {
    ...
    // Compare as local YYYY-MM-DD strings to avoid UTC-offset false positives
    // (new Date("YYYY-MM-DD") parses as UTC midnight, which reads as "future"
    // for today in positive-offset timezones). Warn only for tomorrow onward —
    // logging on the current day is expected user behaviour.
    const now = new Date();
    const todayStr = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`;

    if (cell.dataset.date > todayStr) showWarning();
  };
...
// =============================================================================
// PINNED TASKS — v1.9.0
// A separate "favourites" box injected above the task list. It keeps its own
// ordered store (chrome.storage.local, namespaced per workspace host) and
// renders its own rows, so it NEVER fights Scoro's virtualized/lazy-loaded
// task table. Pin buttons are injected per row (idempotent) and survive lazy
// scroll + SPA navigation via a tbody-scoped observer + a low-freq fallback.
// =============================================================================
function initPinnedTasks() {
  const HOST = location.hostname;
  const PINS_KEY = 'scoro_pinned_tasks_' + HOST;
  const COLLAPSE_KEY = 'scoro_pinned_collapsed_' + HOST;
  const GROUP_KEY = 'scoro_pinned_group_' + HOST;
  const COLORS_KEY = 'scoro_pinned_colors_' + HOST;

  let enabled = true;
  let pins = [];
  let collapsed = false;
  let grouped = false;
  let clientColors = {}; // { [clientName]: hex } — user overrides of the auto colour
  ...
  function loadState() {
    return new Promise((res) => {
      if (!alive()) return res();
      chrome.storage.local.get([PINS_KEY, COLLAPSE_KEY, GROUP_KEY, COLORS_KEY], (r) => {
        pins = Array.isArray(r[PINS_KEY]) ? r[PINS_KEY] : [];
        collapsed = !!r[COLLAPSE_KEY];
        grouped = !!r[GROUP_KEY];
        clientColors = (r[COLORS_KEY] && typeof r[COLORS_KEY] === 'object') ? r[COLORS_KEY] : {};
        res();
      });
    });
  }
  function savePins() { if (alive()) chrome.storage.local.set({ [PINS_KEY]: pins }); }
  function saveCollapsed() { if (alive()) chrome.storage.local.set({ [COLLAPSE_KEY]: collapsed }); }
  function saveGrouped() { if (alive()) chrome.storage.local.set({ [GROUP_KEY]: grouped }); }
  function saveClientColors() { if (alive()) chrome.storage.local.set({ [COLORS_KEY]: clientColors }); }

  // Deterministic accent colour per client (stable across sessions). A vibrant,
  // harmonious 16-hue set built from the Open Color scheme (yeun.github.io/open-color),
  // spanning the wheel with 3 blues and 3 greens as light→deep shades.
  ...
  function clientColor(name) {
    const s = (name || '').trim() || 'No client';
    if (clientColors[s]) return clientColors[s]; // user override wins
    if (s === 'No client') return '#c9ced6';
    let h = 5381;
    for (let i = 0; i < s.length; i++) h = ((h * 33) ^ s.charCodeAt(i)) >>> 0;
    return CLIENT_PALETTE[h % CLIENT_PALETTE.length];
  }

  // ---------- per-client colour picker (small palette popover) ----------
  function closeColorPop() {
    const ex = document.getElementById('scoro-ext-color-pop');
    if (ex) ex.remove();
    document.removeEventListener('mousedown', onDocDownForPop, true);
  }
  function onDocDownForPop(e) {
    const pop = document.getElementById('scoro-ext-color-pop');
    if (pop && !pop.contains(e.target) && !(e.target.classList && e.target.classList.contains('scoro-ext-pinned-swatch')))
      closeColorPop();
  }
  function openColorPop(client, anchor) {
    const already = document.getElementById('scoro-ext-color-pop');
    closeColorPop();
    if (already && already.getAttribute('data-client') === client) return; // toggle off
    const pop = document.createElement('div');
    pop.id = 'scoro-ext-color-pop';
    pop.className = 'scoro-ext-color-pop';
    pop.setAttribute('data-client', client);
    const current = (clientColors[client] || '').toLowerCase();
    CLIENT_PALETTE.forEach((c) => {
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'scoro-ext-color-opt' + (current === c.toLowerCase() ? ' sel' : '');
    ...
    auto.type = 'button';
    auto.className = 'scoro-ext-color-auto';
    auto.textContent = 'Auto colour';
    ...
    if (pr.right > window.innerWidth - 8) pop.style.left = Math.round(window.innerWidth - pr.width - 8) + 'px';
    setTimeout(() => document.addEventListener('mousedown', onDocDownForPop, true), 0);
  }

  // Bucket pins by client, preserving each client's first-appearance order and
  // the stored order within a client. Used for the "group by client" view.
  function groupPins() {
    const groups = [];
    const idx = {};
    pins.forEach((p) => {
      const c = (p.client || '').trim() || 'No client';
      if (idx[c] === undefined) { idx[c] = groups.length; groups.push({ client: c, items: [] }); }
      groups[idx[c]].items.push(p);
    });
    return groups;
  }
  ...
  const onListPage = () => !!table();
  const isPinned = (id) => pins.some((p) => p.id === String(id));

  function rowId(tr) {
    return (
      tr.getAttribute('data-task-id') ||
      tr.getAttribute('data-node-id') ||
      ((tr.querySelector('a[href*="/tasks/view/"]') || {}).getAttribute &&
        (tr.querySelector('a[href*="/tasks/view/"]').getAttribute('href') || '').match(
          /\/tasks\/view\/(\d+)/
        )?.[1]) ||
      null
    );
  ...
  function readRow(tr) {
    const id = rowId(tr);
    if (!id) return null;
    const a = tr.querySelector('a[href*="/tasks/view/"]');
    const name = ((a && (a.getAttribute('title') || a.textContent)) || 'Task #' + id)
      .replace(/\s+/g, ' ').trim();
    // Mirror the list row: client (col_multi_1) on the left, status pill on the
    // right. Capture the pill's computed colours so it matches Scoro exactly.
    const clientEl = tr.querySelector('td.col_multi_1');
    const client = clientEl ? clientEl.textContent.replace(/\s+/g, ' ').trim() : '';
    const statusEl = tr.querySelector('td.colStatus .js-status-name-full');
    const pillEl = tr.querySelector('td.colStatus .colstatus');
    ...
    return {
      id: String(id), name, url: location.origin + '/tasks/view/' + id,
      client, status, statusBg, statusColor,
    };
  }

  // ---------- one-time style ----------
  function ensureStyle() {
    if (document.getElementById('scoro-ext-pinned-style')) return;
    const s = document.createElement('style');
    s.id = 'scoro-ext-pinned-style';
    s.textContent = `
      #scoro-pinned-box{margin:0 0 12px;border:1px solid #e3e6ea;border-radius:8px;
        background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.04);font-size:13px;overflow:hidden}
      .scoro-ext-pinned-header{display:flex;align-items:center;justify-content:space-between;
        padding:8px 12px;cursor:pointer;user-select:none;background:#f7f8fa;border-bottom:1px solid #eceef1}
      .scoro-ext-pinned-title{font-weight:600;color:#2c3038;display:flex;align-items:center;gap:6px}
      .scoro-ext-pin-star{color:#f5a623}
      .scoro-ext-pinned-count{background:#e6eef9;color:#1a73e8;border-radius:10px;padding:0 7px;
        font-size:11px;font-weight:600}
      .scoro-ext-pinned-toggle{border:none;background:none;cursor:pointer;font-size:13px;color:#6b7280;
        padding:2px 8px;border-radius:4px;line-height:1}
      .scoro-ext-pinned-toggle:hover{background:#e9ebef}
      .scoro-ext-pinned-header-right{display:flex;align-items:center;gap:10px}
      .scoro-ext-pinned-group-toggle{border:1px solid #d4d8de;background:#fff;color:#6b7280;cursor:pointer;
        font-size:11px;font-weight:600;padding:3px 10px;border-radius:12px;line-height:1.4}
      .scoro-ext-pinned-group-toggle:hover{background:#eef1f5}
      .scoro-ext-pinned-group-toggle.on{background:#e6eef9;border-color:#b6d0f5;color:#1a73e8}
      .scoro-ext-pinned-list{padding:4px 0}
      .scoro-ext-pinned-empty{padding:10px 16px;color:#8a9099;font-style:italic}
      /* Grouped: a thin client-coloured rail down each section, then the client
         name + count and a hairline rule to the right edge (label + divider). */
      .scoro-ext-pinned-group-section{border-left:3px solid transparent;margin-top:2px}
      .scoro-ext-pinned-list > .scoro-ext-pinned-group-section:first-child{margin-top:0}
      .scoro-ext-pinned-group{display:flex;align-items:center;gap:8px;padding:11px 14px 5px}
      .scoro-ext-pinned-swatch{flex:0 0 auto;width:12px;height:12px;padding:0;border-radius:50%;
        border:1px solid rgba(0,0,0,.14);cursor:pointer;transition:transform .1s}
      .scoro-ext-pinned-swatch:hover{transform:scale(1.18)}
      .scoro-ext-pinned-group-name{font-size:12px;font-weight:600;color:#3c4149;white-space:nowrap}
      .scoro-ext-pinned-group-count{font-size:11px;font-weight:600;color:#aab0b8}
      .scoro-ext-pinned-group-rule{flex:1;height:1px;background:#ececf1;margin-left:2px}
      .scoro-ext-color-pop{position:fixed;z-index:2147483000;background:#fff;border:1px solid #e3e6ea;
        border-radius:10px;box-shadow:0 8px 28px rgba(0,0,0,.16);padding:9px;
        display:grid;grid-template-columns:repeat(8,20px);gap:7px}
      .scoro-ext-color-opt{width:20px;height:20px;padding:0;border-radius:50%;border:2px solid transparent;
        cursor:pointer;transition:transform .1s}
      .scoro-ext-color-opt:hover{transform:scale(1.15)}
      .scoro-ext-color-opt.sel{border-color:#2c3038}
      .scoro-ext-color-auto{grid-column:1 / -1;margin-top:3px;border:1px solid #d4d8de;background:#fff;
        border-radius:6px;font-size:11px;font-weight:600;color:#6b7280;cursor:pointer;padding:5px 0}
      .scoro-ext-color-auto:hover{background:#eef1f5}
      /* Rows: compact, drag dots appear on hover so they don't clutter. */
      .scoro-ext-pinned-row{display:flex;align-items:center;gap:0;padding:7px 16px}
      .scoro-ext-pinned-row:hover{background:#f5f8fd}
      .scoro-ext-pinned-row.dragging{opacity:.4;background:#e6eef9}
      .scoro-ext-pinned-row--grouped{padding-left:30px}
      .scoro-ext-pinned-handle{flex:0 0 16px;cursor:grab;color:#c9ced6;font-size:12px;letter-spacing:-2px;
        opacity:0;transition:opacity .1s}
      .scoro-ext-pinned-row:hover .scoro-ext-pinned-handle{opacity:1}
      .scoro-ext-pinned-client{flex:0 0 150px;margin:0 16px 0 8px;color:#8a9099;
        white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .scoro-ext-pinned-row--grouped .scoro-ext-pinned-link{margin-left:8px}
      .scoro-ext-pinned-link{flex:1;min-width:80px;color:#1a73e8;text-decoration:none;white-space:nowrap;
        overflow:hidden;text-overflow:ellipsis}
      .scoro-ext-pinned-link:hover{text-decoration:underline}
      .scoro-ext-pinned-status{flex:0 0 auto;margin-left:14px;font-size:11px;font-weight:600;line-height:1;
        padding:3px 9px;border-radius:10px;white-space:nowrap;color:#fff}
      .scoro-ext-pinned-remove{border:none;background:none;color:#b0b4bb;cursor:pointer;font-size:12px;
        margin-left:10px;padding:2px 6px;border-radius:4px;opacity:0;transition:opacity .1s}
      .scoro-ext-pinned-row:hover .scoro-ext-pinned-remove{opacity:1}
      .scoro-ext-pinned-remove:hover{background:#fde8e8;color:#e53935}
      .scoro-ext-pin-btn{cursor:pointer;color:#c9ced6;margin-right:6px;font-size:14px;
        vertical-align:middle;display:inline-block}
      .scoro-ext-pin-btn:hover{color:#f5a623}
      .scoro-ext-pin-btn.pinned{color:#f5a623}
    `;
  ...
  function updateBtn(btn, on) {
    btn.textContent = on ? '★' : '☆';
    btn.title = on ? 'Unpin task' : 'Pin task';
    btn.classList.toggle('pinned', on);
  }
  function decorateRows() {
    const t = table();
    if (!t) return;
    // The task list splits rows across MANY tbodies (lazy loader appends new
    // tbodies on scroll), so query the whole table, not just tBodies[0].
    t.querySelectorAll('tr.TaskRow:not([data-pin-added])').forEach((tr) => {
      tr.setAttribute('data-pin-added', '1');
      const id = rowId(tr);
      if (!id) return;
      const btn = document.createElement('span');
      btn.className = 'scoro-ext-pin-btn';
      btn.setAttribute('data-pin-id', id);
      updateBtn(btn, isPinned(id));
      // Place the star inline, immediately before the title link, so it shares
      // the same line. (Inserting at the cell's start put it before the
      // block-level .eventNameWrapper, which pushed the title onto a 2nd row.)
      const a = tr.querySelector('a[href*="/tasks/view/"]');
      if (a && a.parentNode) {
  ...
  function refreshRowButtons() {
    document.querySelectorAll('.scoro-ext-pin-btn').forEach((btn) =>
      updateBtn(btn, isPinned(btn.getAttribute('data-pin-id')))
    );
  }

  // Keep a pinned task's stored snapshot (name/client/status/colours) in sync
  // with its live list row. Pills self-heal whenever the row is in the DOM —
  // on load or as lazy-scroll brings it into view. Returns true if it changed.
  function syncPinFromRow(tr) {
    const id = rowId(tr);
    if (!id || !isPinned(id)) return false;
    const pin = pins.find((p) => p.id === String(id));
    const fresh = readRow(tr);
    if (!pin || !fresh) return false;
    let changed = false;
    ['name', 'client', 'status', 'statusBg', 'statusColor'].forEach((k) => {
      if (fresh[k] && pin[k] !== fresh[k]) { pin[k] = fresh[k]; changed = true; }
    });
    return changed;
  }
  function refreshPinnedFromList() {
    const t = table();
    if (!t || !pins.length) return;
    let changed = false;
    t.querySelectorAll('tr.TaskRow').forEach((tr) => { if (syncPinFromRow(tr)) changed = true; });
    if (changed) { savePins(); renderBox(true); }
  }

  // ---------- the box ----------
  function ensureBox() {
    let box = document.getElementById('scoro-pinned-box');
    if (box && box.isConnected) return box;
    const host = document.getElementById('leftTasksContent');
    const anchor = document.querySelector('.tasksContainer');
    if (!host || !anchor) return null;
    box = document.createElement('div');
    box.id = 'scoro-pinned-box';
    host.insertBefore(box, anchor);
    return box;
  }

  function renderBox(force) {
    if (!enabled) return;
    const box = ensureBox();
    if (!box) return;
    const key = JSON.stringify({ pins, collapsed, grouped, clientColors });
    if (!force && key === lastRenderKey && box.childElementCount) return;
    lastRenderKey = key;
    box.innerHTML = '';

    const header = document.createElement('div');
    header.className = 'scoro-ext-pinned-header';
    const title = document.createElement('span');
    title.className = 'scoro-ext-pinned-title';
    title.innerHTML =
      '<span class="scoro-ext-pin-star">★</span> Pinned tasks' +
      (pins.length ? ' <span class="scoro-ext-pinned-count">' + pins.length + '</span>' : '');
    // Group-by-client toggle. stopPropagation so it doesn't collapse the box.
    const groupBtn = document.createElement('button');
    groupBtn.className = 'scoro-ext-pinned-group-toggle' + (grouped ? ' on' : '');
    groupBtn.type = 'button';
    groupBtn.title = grouped ? 'Grouping by client — click to ungroup' : 'Group by client';
    groupBtn.textContent = 'Group by client';
    ...
    chevron.className = 'scoro-ext-pinned-toggle';
    chevron.type = 'button';
    chevron.title = collapsed ? 'Expand' : 'Collapse';
    chevron.textContent = collapsed ? '▸' : '▾';
    const right = document.createElement('span');
    right.className = 'scoro-ext-pinned-header-right';
    ...
    list.className = 'scoro-ext-pinned-list';
    if (!pins.length) {
      const empty = document.createElement('div');
      empty.className = 'scoro-ext-pinned-empty';
      empty.textContent = 'No pinned tasks yet. Click the ☆ on any task to pin it here.';
      list.appendChild(empty);
    } else if (grouped) {
      groupPins().forEach((g) => {
        const section = document.createElement('div');
        section.className = 'scoro-ext-pinned-group-section';
        section.style.borderLeftColor = clientColor(g.client);
        const gh = document.createElement('div');
        gh.className = 'scoro-ext-pinned-group';
        const swatch = document.createElement('button');
        swatch.type = 'button';
        swatch.className = 'scoro-ext-pinned-swatch';
        swatch.style.background = clientColor(g.client);
        swatch.title = 'Set colour for ' + g.client;
        swatch.addEventListener('click', (e) => {
          e.stopPropagation();
          openColorPop(g.client, swatch);
        });
        const gn = document.createElement('span');
        gn.className = 'scoro-ext-pinned-group-name';
        gn.textContent = g.client;
        const gc = document.createElement('span');
        gc.className = 'scoro-ext-pinned-group-count';
        gc.textContent = g.items.length;
        const gr = document.createElement('span');
        gr.className = 'scoro-ext-pinned-group-rule';
        gh.appendChild(swatch);
        gh.appendChild(gn);
        gh.appendChild(gc);
        gh.appendChild(gr);
        section.appendChild(gh);
        g.items.forEach((p) => section.appendChild(renderPinRow(p)));
        list.appendChild(section);
      });
    } else {
      pins.forEach((p) => list.appendChild(renderPinRow(p)));
    }
  ...
  function renderPinRow(p) {
    ...
    row.className = 'scoro-ext-pinned-row' + (grouped ? ' scoro-ext-pinned-row--grouped' : '');
    row.setAttribute('draggable', 'true');
    row.setAttribute('data-pin-id', p.id);
    row.setAttribute('data-client', (p.client || '').trim() || 'No client');

    const handle = document.createElement('span');
    handle.className = 'scoro-ext-pinned-handle';
    handle.textContent = '⋮⋮';
    handle.title = 'Drag to reorder';

    // Ungrouped shows the client column; grouped shows it in the section header.
    // No title attributes on row content — they produce native hover tooltips.
    const client = document.createElement('span');
    client.className = 'scoro-ext-pinned-client';
    client.textContent = p.client || '';

    const link = document.createElement('a');
    link.className = 'scoro-ext-pinned-link';
    link.href = p.url;
    link.textContent = p.name;

    const status = document.createElement('span');
    status.className = 'scoro-ext-pinned-status';
    if (p.status) {
      status.textContent = p.status;
      if (p.statusBg) status.style.background = p.statusBg;
      if (p.statusColor) status.style.color = p.statusColor;
    } else {
      status.style.display = 'none';
    }

    const del = document.createElement('button');
    del.className = 'scoro-ext-pinned-remove';
    del.type = 'button';
    del.textContent = '✕';
    del.title = 'Unpin';
    ...
    if (!grouped) row.appendChild(client);
    ...
    row.addEventListener('dragover', (e) => {
      e.preventDefault();
      const dragging = document.querySelector('.scoro-ext-pinned-row.dragging');
      if (!dragging || dragging === row) return;
      // When grouped, only reorder within the same client group.
      if (grouped && dragging.getAttribute('data-client') !== row.getAttribute('data-client')) return;
    ...
    row.addEventListener('drop', (e) => {
      e.preventDefault();
      const ids = [...document.querySelectorAll('.scoro-ext-pinned-row')].map((r) =>
        r.getAttribute('data-pin-id')
      );
      pins.sort((a, b) => ids.indexOf(a.id) - ids.indexOf(b.id));
      savePins();
      lastRenderKey = '';
      renderBox(true);
    });
  ...
  // ---------- teardown ----------
  function removeAll() {
    const box = document.getElementById('scoro-pinned-box');
    if (box) box.remove();
    document.querySelectorAll('.scoro-ext-pin-btn').forEach((b) => b.remove());
    document
      .querySelectorAll('tr.TaskRow[data-pin-added]')
      .forEach((tr) => tr.removeAttribute('data-pin-added'));
    lastRenderKey = '';
  }
  ...
  function observeRows() {
    const t = table();
    if (!t) return;
    // Observe the whole table (subtree) so rows in tbodies appended later by
    // the lazy loader also get decorated.
    if (rowObserver && rowObserver._el === t) return;
    if (rowObserver) rowObserver.disconnect();
    rowObserver = new MutationObserver(() => decorateRows());
    rowObserver._el = t;
    rowObserver.observe(t, { childList: true, subtree: true });
  }
  function sync() {
    if (!enabled) return;
    if (!onListPage()) {
  ...
  // ---------- pin click (delegated, capture: beats row navigation) ----------
  document.addEventListener(
    'click',
    (e) => {
      const btn = e.target.closest && e.target.closest('.scoro-ext-pin-btn');
      if (!btn) return;
      e.preventDefault();
      e.stopPropagation();
      const id = btn.getAttribute('data-pin-id');
      const tr = btn.closest('tr.TaskRow');
      if (isPinned(id)) {
        pins = pins.filter((p) => p.id !== id);
      } else if (tr) {
        const d = readRow(tr);
        if (d && !isPinned(d.id)) pins.push(d);
      }
      savePins();
      refreshRowButtons();
      renderBox(true);
  ...
  // ---------- popup toggle ----------
  // Pinned tasks ride the main "UI Refresh" switch — no separate toggle.
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type !== 'UPDATE_SETTINGS' || msg.setting !== 'uiRefreshEnabled') return;
    enabled = msg.value;
    if (enabled) sync();
    else removeAll();
  });
  ...
  loadState().then(() => {
    if (!alive()) return;
    chrome.storage.sync.get(['uiRefreshEnabled'], (res) => {
      enabled = res.uiRefreshEnabled ?? true;
      if (!enabled) return;
      const boot = () => {
        sync();
