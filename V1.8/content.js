/**
 * Scoro UI Refresh — Content Script (V1.8.0)
 *
 ...
 *                             Product rows: tr[nobr], qty in td[2], unit in td[3].
...
function injectOfferHourSummaries() {
  ...
  document.querySelectorAll('.scoro-ext-hour-summary, .scoro-ext-grand-total').forEach(el => el.remove());
...
function initOfferHourSummaries() {
  ...
  // Re-inject when user edits quantity, unit, or internal hours
  document.addEventListener('input', (e) => {
    const n = e.target?.name || '';
    if (n === 'paarotLines[][amount]' || n === 'paarotLines[][unit_name]' || n === 'paarotLines[][custom_field][c_internalhours]') {
...
function injectOfferHourSummariesView() {
  ...
  for (const row of rows) {
    ...
    if (isNobr && row.querySelector('td h4')) {
      ...
      // Product row — qty in td[2], unit in td[3]
